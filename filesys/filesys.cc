...
FileSystem::FileSystem() 
{
    openingFile = new OpenFile *[MAX_FILES];
    ...
    openingFile[0] = this->Open("Console Input", 0);
    openingFile[1] = this->Open("Console Output", 0);
}
...
OpenFile *FileSystem::GetFileDescriptor(char *fileName) 
{
    for (int i = 0; i < MAX_FILES; i++) 
    {
        if (openingFile[i] != NULL) 
        {
            char *buffer = openingFile[i]->GetFileName();
